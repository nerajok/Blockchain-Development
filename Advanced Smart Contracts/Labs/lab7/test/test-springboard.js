...
contract("Springboard", accounts => {
   ...
   it("Upgrade wallet v1 to v2 should work", async () => {
      ...
      // 1) Upgrade wallet to V2
		await walletV1.die();
		const wal = WalletV2.deployedBytecode;	  
		let upgrade = await springboard.execute(wal);
      // 2) verify wallet version == 2.0 after upgrade
        const newWallet = upgrade.logs[0].args[0];
		assert.equal(walletAddress, newWallet, "address mismatch");
    
		const check = await WalletV2.at(newWallet);
		let newVersion = await check.version();
		assert.equal(newVersion, "2.0", "version error");
