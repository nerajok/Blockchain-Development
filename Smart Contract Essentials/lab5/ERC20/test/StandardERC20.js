...
contract("StandardERC20", (accounts) => {
  ...
  const receipient1Amount = web3.utils.toBN(5000000000000000000);
  ...
  const oldSpenderAllowance = await standardERC20Instance.allowance.call(
    ...
    truffleAssert.eventEmitted(increaseAllowance, "Approval", (obj) => {
      return (
        obj.owner === receipient1 &&
        obj.spender === spender &&
        new BigNumber(obj.value).isEqualTo(oldSpenderAllowance.add(spenderAmount))
      );
    });
    ...
    const spenderNewAllowance = await standardERC20Instance.allowance.call(
      receipient1,
      spender
    );
  
  const expectedSpenderAllowance = web3.utils
      .toBN(oldSpenderAllowance)
      .add(spenderAmount);
  assert(
    increaseAllowance &&
      new BigNumber(spenderNewAllowance).isEqualTo(
...
it("test decreaseAllowance()", async () => {
  const oldSpenderAllowance = await standardERC20Instance.allowance.call(
    ...
    truffleAssert.eventEmitted(decreaseAllowance, "Approval", (obj) => {
      return (
        obj.owner === receipient1 &&
        obj.spender === spender &&
        new BigNumber(obj.value).isEqualTo(oldSpenderAllowance.sub(spenderAmount))
      );
    });
